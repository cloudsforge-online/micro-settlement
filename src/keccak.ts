 ...
 *
 * ## What it is for in THIS service, both of which are money
 *
 ...
 *      `evm.ts` says the rest.
 *
 * Carried across from `wallet/src/keccak.ts` unchanged, because two services deriving the same
 * address checksum from two implementations is a difference that would present as a withdrawal
 * refused for a destination the user copied out of our own UI.
 */
